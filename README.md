...
下一阶段会在现有指挥层之上实现真正的 CEO Agent。相比手动组织工作流，golutra 会逐步走向长期运行的自动化协调中枢，能够精确构建子 Agent、管理分层记忆，并在极少人工监管下持续推动多智能体系统运转。

即将加入的能力包括：

- CEO Agent —— 真正的顶层调度者，目标是一个月不用人监管，能够精确构建子 Agent，并在不同角色与任务之间实现记忆分层。

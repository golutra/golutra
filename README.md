...
Many planned features are still under development, so this project is currently not open source.

In the future, open-sourcing will be reconsidered if maintenance becomes unsustainable for a single developer, or if a commercial edition roadmap makes open-sourcing a better long-term strategy.
...
- Restrictions apply only to `golutra` itself (including derivative versions). Without prior written authorization, re-development, redistribution, resale, or other commercial distribution of `golutra` is prohibited.
...
目前仍有较多规划中的功能尚未实现，因此项目暂不开源。

后续如果个人维护压力过大难以持续，或出现更适合开源的商业版规划，会重新评估并选择开源。

### 使用许可

- 允许将 `golutra` 作为工具用于商业软件开发。
- 用户通过 `golutra` 产出的代码与交付成果归用户所有。
- 限制仅针对 `golutra` 本体（含衍生版本）。未经事先书面授权，不得对 `golutra` 进行再开发、再分发、转售或其他商业化分发。
